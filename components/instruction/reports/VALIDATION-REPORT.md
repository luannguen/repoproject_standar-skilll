...
| `skill-lint.ps1` | PASS - 0 blockers, 0 warnings | 22 Standard Skills, contracts, versions, paths, dependencies, workflow use |
| `workflow-lint.ps1` | PASS - 0 blockers, 0 warnings | 12 workflows, gates, skills, paths, completion contracts |
| `habit-lint.ps1` | PASS - 0 blockers, 0 warnings | default-deny registry, lifecycle and authority contract, one valid fixture, one adversarial fixture with expected rejection reasons |
...
| `memory-lint.ps1` | PASS - 21 entries | indexed details, IDs, sources, lifecycle, secret heuristics |
...
- Procedural Memory defaults deny with zero registered/selectable habits in template mode; the route example exists only as test data.
- 22 Standard Skills, 12 primary workflows, governed VOPL Procedural Memory, and 27 reusable Custom Skills remain separate.
